...
void Screensaver::onInit()
{
    ...
    _time_label->setTextFont(&lv_font_montserrat_24);
    applyLabelColors();
...
void Screensaver::onBuildLevel()
{
    ...
    const float logo_w    = static_cast<float>(_logo_width);
    const float logo_h    = static_cast<float>(_logo_height);
    const Vector2 logo_size = {logo_w, logo_h};
