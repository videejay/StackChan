...
void FaceDetector::processFrame()
{
...
#if CONFIG_HUMAN_FACE_DETECT_MODEL_IN_FLASH_PARTITION
    // FbsLoader resolves label "human_face_det" (esp-dl). Missing partition → panic on first run().
    static bool s_checked_face_part = false;
    static bool s_have_face_model_part = false;
    if (!s_checked_face_part) {
        s_checked_face_part = true;
        s_have_face_model_part =
            esp_partition_find_first(ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY, "human_face_det") != nullptr;
        if (!s_have_face_model_part) {
            ESP_LOGE(TAG,
                     "Partition \"human_face_det\" missing — face inference disabled. "
                     "Flash partition table + model, or switch to CONFIG_HUMAN_FACE_DETECT_MODEL_IN_SDCARD.");
        }
    }
    if (!s_have_face_model_part) {
        arbiter.releaseForDetection();
    ...
    // Same layout as launcher SD assets: <STACKCHAN_SD_MOUNT_PATH>/<MODEL_SDCARD_DIR>/<model>.espdl
    ...
    if (!s_checked_sd_models) {
        ...
        const char* sd_root = CONFIG_BSP_SD_MOUNT_POINT;
#else
        ...
        if (!s_have_sd_models) {
            ESP_LOGE(TAG,
                     "Face MSR model not on SD (expected %s). Copy human_face_detect_*_s8_v1.espdl from "
                     "managed_components/espressif__human_face_detect/models/s3/ into %s/%s/",
                     path,
                     sd_root,
                     CONFIG_HUMAN_FACE_DETECT_MODEL_SDCARD_DIR);
        }
