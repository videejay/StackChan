...
public:
    ...
    void update()
    {
        ...
        if (_is_streaming) {
            const uint32_t interval = _is_video_mode ? 700 : 350;
            if (avatarCameraStreamTick(interval)) {
                captureAndSendFrame();
            }
        }
    }
    ...
    void captureAndSendFrame()
    {
        ...
        if (!avatarCameraStreamCopyLatestJpeg(jpeg) || jpeg.empty()) {
            return;
        }

        sendPacket(DataType::Jpeg, jpeg.data(), jpeg.size());
    }
